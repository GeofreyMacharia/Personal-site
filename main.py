...
@st.cache_data()
def get_base64_of_bin_file6(bin_file):
    with open(bin_file, 'rb') as f:
        data = f.read()
    return base64.b64encode(data).decode()

def set_png_as_page_bg6(png_file):
    bin_str6 = get_base64_of_bin_file(png_file)
    page_bg_img6 = '''
    <style>
    .first_back {
    background-image: url("data:image/png;base64,%s");
    ...
    st.markdown(page_bg_img6, unsafe_allow_html=True)
    return

def set_png_as_page_bg7(png_file):
    bin_str7 = get_base64_of_bin_file(png_file)
    page_bg_img7 = '''
    <style>
    .second_back {
    background-image: url("data:image/png;base64,%s");
    ...
    st.markdown(page_bg_img7, unsafe_allow_html=True)
    return
...
with tab1:
    st.markdown('''<div class="second_back"> </div>''', unsafe_allow_html=True)
    set_png_as_page_bg7('images//background2.png')


    st.markdown('''<div class="first_back"> </div>''', unsafe_allow_html=True)
    set_png_as_page_bg6('images//background.png')
    st.markdown('''<div class="My_face2"></div>''',unsafe_allow_html=True)
    set_png_as_page_bg3('images//Profile_face.png')
    st.markdown('''<div class="My_name2"> Geofrey Macharia</div>''',unsafe_allow_html=True)
    st.markdown('''<div class="My_work2">Data scientist | Software Engineer | Data Analyst </div>''',unsafe_allow_html=True)
    
    
    st.markdown('''<div class="greetings"><b>Hello</b>,</div>''',unsafe_allow_html=True)
    st.markdown('''<div class="intro2">glad to have you drop by :)</div>''', unsafe_allow_html=True)
    st.markdown('''<div class="Me2"><b>Me, Myself and I </b></div>''', unsafe_allow_html=True)
    st.markdown('''<div class="home_details2"> I am a passionate engineer with the expertise and desire for providing insight and solving problems.
    ...
    st.markdown('''
            <div class="GitHub2">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width=26 height=26 fill="currentColor">
...
with tab3:
    ...
    with project_con:
        ...
        with proj4:
            ...
            <a href="https://github.com/GeofreyMacharia/"><b>Further details</b></a></div>''', unsafe_allow_html=True)
